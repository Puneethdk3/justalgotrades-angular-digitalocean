...
;( function( $ ) {
    ...
    "use strict";
        ...
        function Plugin( element, data, options ) {
            ...
            this.data = data;
            defaults.lastYear  = parseInt(options.title)+1;
            defaults.lastMonth = 0;
        ...
        $.extend( Plugin.prototype, {
                            ...
                            if ( this.data[ 0 ].date && this.data[ 0 ].count ) {
                                ...
                                for ( var h in this.data ) {
                                    ...
                                    arr.push( {
                                        ...
                                        "profitGot": this.data[h].profitGot,
                                        "daysProfit": this.data[h].daysProfit
                                    } );
                                }
                                return arr;
                            } else {
            ...
            calculateBins: function( events ) {
                ...
                var binLabelsTemp = binlabels;
                binlabels = [];
                for(var i=binLabelsTemp.length-1; i >=0; i--)
                {
                    l = binLabelsTemp[i];
                    var negBins = [] ;
                    negBins = l.split(" "); 
                    if(negBins.length ==3)
                    {
                        binlabels.push( String( negBins[0]*-10+" "+negBins[1]+" "+negBins[2]*-10 ) );
                    }
                    if(l.includes("+"))
                    {
                        l = l.replace("+", "");
                        binlabels.push( String("<"+l*-10) );
                    }
                }
                for(i in binLabelsTemp)
                {
                    l = binLabelsTemp[i];
                    var negBins = [] ;
                    negBins = l.split(" "); 
            ...
            matchBin: function( range, value ) {
                for ( var r in range ) {
                    ...
                    if ( value <= range[ r ][ 0 ]*-1 && value >= range[ r ][ 1 ]*-1 ) {
                        return parseInt(r)+5;
                    }
                }
                return 0;
            ...
            calendarHeatmap: function( ) {
                ...
                for ( i = months; i > 0; i-- ) {
                    ...
                    for ( var j = 0; j < days; j++ ) {
                        ...
                        if ( obj ) {
                            var title = obj.daysProfit + " on ";
                            title += moment( obj.date ).format( "ll" );

                            var color = "";

                            if ( this.settings.coloring ) {
                                if(parseInt(obj.level)==5) {obj.level = 0;color = "red";}
                                if(parseInt(obj.level)==6) {obj.level = 1;color = "red";}
                                if(parseInt(obj.level)==7) {obj.level = 2;color = "red";}
                                if(parseInt(obj.level)==8) {obj.level = 3;color = "red";}
                                if(parseInt(obj.level)==9) {obj.level = 4;color = "red";}
                                // color = " " + this.settings.coloring + "-" + obj.level;
                                color = " " + color + "-" + obj.level;
                            }
                ...
                if ( this.settings.legend.show ) {
                    ...
                    for ( i = 0; i < binLabels.length; i++ ) {
                        
                        {
                            ...
                            if(i==0) {
                                colorclass= "ch-lvl lvl-1";
                                color = "red-4";
                            }
                            else if(i==1) {
                                colorclass= "ch-lvl lvl-2";
                                color = "red-3";
                            }
                            else if(i==2) {
                                colorclass= "ch-lvl lvl-3";
                                color = "red-2";
                            }
                            else if(i==3) {
                                colorclass= "ch-lvl lvl-4";
                                color = "red-1";
                            }
                            else if(i==4) {
                                colorclass= "ch-lvl lvl-2";
